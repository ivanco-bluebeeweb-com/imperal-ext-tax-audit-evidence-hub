...
|---|---|---|---|---|---|
| 7 Aug 2026 | Local workspace | P0 build, validation and tests | Passed | `imperal build .` succeeded; `imperal validate .` reported no issues; `../.venv/bin/python -m pytest -q` reported 3 passed | Register app, then deploy |
| 7 Aug 2026 | Imperal Developer Catalog | Deploy `tax-audit-evidence-hub` | Blocked before deployment | Deployment returned `App not found`: the app is not registered in the developer catalog. The local project also has no Git repository/remote, while Developer registration requires a real HTTPS Git URL. | Publish the source in a Git repository, register the app with its HTTPS Git URL, then retry deploy |
| Pending after registration | Imperal panel | Full fictional DemoCo P0 journey | Not started | App cannot be installed until registration and deployment succeed | Complete deploy/install, then run the mandatory live test |
